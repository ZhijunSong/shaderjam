...
// sandbox.load(frag);
// sandbox.setUniform("image","src/test_img.jpg");
...
void main() {
    ...
    // uv += 0.5;
		...
		float strength= smoothstep(0.,1.0,uv.y);
		vec2 surface =strength*vec2(
		mix(-.3,.3,fbm(5.0*uv+u_time*.5)),
		mix(-.3,.3,fbm(5.0*uv+u_time*.5)));
		uv+=refract(vec2(0.0,0.0),surface,1.0/1.333);
	//	 vec4 color= texture(image,uv); 
// gl_FragColor = vec4(2.0,0.0,.0,2.0);
...
async function main() {
    ...
    const vertex_shader_source = VERTEX_SHADER;
    console.log(vertex_shader_source);

    const fragment_shader_source = FRAGMENT_SHADER;
    ...
    const texture = loadTexture(gl, 'https://raw.githubusercontent.com/ZhijunSong/zhijunsong.github.io/main/GISL/src/test_img.jpg', gl.MIRROR_REPEAT, gl.LINEAR_MIPMAP_LINEAR);
